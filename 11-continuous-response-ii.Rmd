...
<!-- * What is the "offset" for a 60-year-old male subject? -->
<!-- * How would you interpret the interaction effect between age and gender for male subjects? -->
...
income_model_interaction <- lm(income ~ age * gender, data = earnings)

# Get regression table:

income_model_interaction %>%
  tidy(conf.int = TRUE) %>%
  select(term, estimate, conf.low, conf.high)
```
```{r echo=FALSE}
...
This holds similarly for `age`. It is the slope for age *for only the female subjects*. Thus, the regression line in the model has an intercept of 4.883 and slope for age of -0.018. Remember that for this data, while the intercept has a mathematical interpretation, it has no *practical* interpretation since subjects can't have zero age.

What about the intercept and slope for age of the male subjects? This is where our notion of "offsets" comes into play once again. The value for `gendermale` is the *offset* in intercept for male subjects relative to female subjects. The intercept for the male subjects is `intercept + gendermale` = 4.883 + (-0.446) = 4.883 - 0.446 = 4.437. Similarly, `age:gendermale` = 0.014 is not the slope for age for the male subjects, but rather the *offset* in slope for the male subjects. Therefore, the slope for age for the male subjects is `age + age:gendermale` $=  -0.018 + 0.014 = -0.004$.
...
Since the slope for age for the female subjects was -0.018, it means that on average, a female subject who is a year older would have a teaching score that is 0.018 units **lower**, whereas the corresponding associated decrease for males was on average only 0.004 units. This is consistent with our observation from Figure \@ref(fig:numxcatxplot1), that this model is suggesting that age impacts teaching scores for female subjects more than for male subjects, as the slope for age for females is more negative than that of males.
...
1. $b_0$ is the `intercept` = 4.883 for the female subjects
1. $b_{\text{age}}$ is the slope for `age` = -0.018 for the female subjects
1. $b_{\text{male}}$ is the offset in intercept = -0.446 for the male subjects
1. $b_{\text{age,male}}$ is the offset in slope for age = 0.014 for the male subjects
...
which is the equation of the `r if_else(knitr::is_latex_output(), "darker-colored", "red")` regression line in Figure \@ref(fig:numxcatxplot1) corresponding to the female subjects in Table \@ref(tab:interaction-summary). Correspondingly, since for male subjects $\mathbb{1}_{\text{is male}}(x)$ = 1, the previous equation becomes
...
which is the equation of the `r if_else(knitr::is_latex_output(), "lighter-colored", "blue")` regression line in Figure \@ref(fig:numxcatxplot1) corresponding to the male subjects in Table \@ref(tab:interaction-summary).
...
Let's put this all together and compute the fitted value $\widehat{y} = \widehat{\text{score}}$ for female subjects. Since for female subjects the indicator function $\mathbb{1}_{\text{is male}}(x)$ = 0, the previous equation becomes
...
Say, you have an subject who identifies as female and is 36 years old. What fitted value $\widehat{y}$ = $\widehat{\text{score}}$ would our model yield? Say, you have another subject who identifies as male and is 59 years old. What would their fitted value $\widehat{y}$ be? 

We answer this question visually first for the female subject by finding the intersection of the `r if_else(knitr::is_latex_output(), "darker-colored", "red")` regression line and the vertical line at $x$ = age = 36. We mark this value with a large `r if_else(knitr::is_latex_output(), "darker-colored", "red")` dot in Figure \@ref(fig:fitted-values). Similarly, we can identify the fitted value $\widehat{y}$ = $\widehat{\text{score}}$ for the male subject by finding the intersection of the `r if_else(knitr::is_latex_output(), "lighter-colored", "blue")` regression line and the vertical line at $x$ = age = 59. We mark this value with a large `r if_else(knitr::is_latex_output(), "lighter-colored", "blue")` dot in Figure \@ref(fig:fitted-values).
...
* For all female subjects: $\widehat{y} = \widehat{\text{score}} = 4.883 - 0.018	\cdot \text{age}$
* For all male subjects: $\widehat{y} = \widehat{\text{score}} = 4.437 - 0.004 \cdot \text{age}$
...
Since our model involves a numerical outcome variable, a numerical explanatory variable, and a categorical explanatory variable, we are in a similar regression modeling situation as in Section \@ref(model4) where we studied the UT Austin teaching scores dataset. Recall in that case the numerical outcome variable was teaching `score`, the numerical explanatory variable was subject `age`, and the categorical explanatory variable was (binary) `gender`.
