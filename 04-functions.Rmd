...
These two functions are what we call built-in functions. We don't do any of the work to code them!
...
mmm(gapminder[c('poverty', 'gdpPercap', 'pop')])
...
quantile(kenya$poverty, probs = 0.5)
median(kenya$poverty)
quantile(kenya$poverty, probs = c(0.25, 0.75))
boxplot(kenya$poverty, plot = FALSE)$stats
```
...
the_quantiles <- quantile(kenya$poverty, probs = the_probs)
...
qdiff1(kenya$poverty, probs = c(0.25, 0.75))
IQR(kenya$poverty)

# Hey, we've reinvented IQR

qdiff1(kenya$poverty, probs = c(0, 1))
mmm(kenya$poverty)
```
...
qdiff2(zeus = kenya$poverty, hera = 0:1)
```
...
```{r}
qdiff3(kenya$poverty)
mmm(kenya$poverty)
qdiff3(kenya$poverty, c(0.1, 0.9))
```
